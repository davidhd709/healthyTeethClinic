...
export class BotService {
  ...
  async processBookingTurn(
    message: string,
    incomingState?: Partial<BotBookingState>,
  ): Promise<BotBookingTurnResponse> {
    ...
    if (state.appointmentId) {
      return {
        reply:
          'Tu cita ya fue confirmada ✅. Si deseas, también puedo ayudarte a agendar una nueva.',
        state,
        missingFields: [],
        completed: true,
        appointment: {
    ...
    if (currentStep === 'patientName') {
      ...
      if (specialists.length === 0) {
        return {
          reply:
            'No encontré especialistas activos para ese servicio ahora mismo. ¿Quieres elegir otro servicio?',
          state: {
            ...state,
            serviceId: undefined,
      ...
      if (
        this.containsAny(normalizedText, ['cualquiera', 'cualquiera esta bien', 'sin preferencia'])
      ) {
        state.specialistId = specialists[0].id;
        state.specialistName = specialists[0].name;
      ...
      if (!parsedDate) {
        return {
          reply:
            'Por favor indícame una fecha válida en formato YYYY-MM-DD (por ejemplo 2026-04-20), o escribe "mañana".',
          state,
          missingFields: this.getMissingFields(state),
          completed: false,
        };
      ...
      if (reason.length < 10) {
        return {
          reply:
            'Cuéntame brevemente el motivo de la consulta (mínimo 10 caracteres), por favor.',
          state,
          missingFields: this.getMissingFields(state),
          completed: false,
        };
    ...
    let appointment: Awaited<ReturnType<typeof this.createAppointment>>;
    try {
      appointment = await this.createAppointment(dto);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'No fue posible agendar la cita';
      return {
  ...
  private getMissingFields(state: BotBookingState): string[] {
    const missing: string[] = [];

    if (!state.patientName) missing.push('patientName');
    if (!state.patientEmail) missing.push('patientEmail');
    if (!state.patientPhone) missing.push('patientPhone');
    if (!state.serviceId) missing.push('serviceId');
    if (!state.specialistId) missing.push('specialistId');
    if (!state.date) missing.push('date');
    if (!state.time) missing.push('time');
    if (!state.reasonForVisit) missing.push('reasonForVisit');

    return missing;
  }

  private async replyForCurrentStep(
    state: BotBookingState,
  ): Promise<BotBookingTurnResponse> {
    const missing = this.getMissingFields(state);
    ...
    if (step === 'time') {
      let availabilityText =
        '¿A qué hora prefieres? (formato HH:mm, por ejemplo 09:30)';
      if (state.specialistId && state.date) {
        const availability = await this.availabilityService.getAvailability(
          state.specialistId,
          state.date,
    ...
    return {
      reply:
        'Cuéntame brevemente el motivo de tu consulta (por ejemplo: control de brackets, dolor molar, limpieza preventiva).',
      state,
      missingFields: missing,
      completed: false,
    };
  ...
  private async replyWithServicesAndKeepStep(
    state: BotBookingState,
  ): Promise<BotBookingTurnResponse> {
    const services = await this.listServices();
  ...
  private async resolveService(query: string): Promise<ResolveResult<BotServiceDto>> {
    const services = await this.listServices();
    ...
    const aliasEntry = Object.entries(this.serviceAliases).find(([alias]) =>
      normalizedQuery.includes(this.normalizeText(alias)),
    );
    if (aliasEntry) {
      const byAlias = services.find((s) => s.slug === aliasEntry[1]);
      if (byAlias) return { value: byAlias };
    }

    const exact = services.find(
      (service) =>
        this.normalizeText(service.name) === normalizedQuery ||
  ...
  private async resolveSpecialist(
    query: string,
    specialists: BotSpecialistDto[],
  ): Promise<ResolveResult<BotSpecialistDto>> {
    if (!query.trim()) return {};
    ...
    const exact = specialists.find(
      (specialist) =>
        this.normalizeText(specialist.name) === normalizedQuery ||
    ...
    const ranked = specialists
      .map((specialist) => {
        ...
        const score =
          (normalizedName.includes(normalizedQuery) ? 2 : 0) +
  ...
  private extractName(text: string): string | null {
    if (this.extractEmail(text)) return null;
  ...
  private extractDate(text: string): string | null {
    ...
    if (isoMatch) {
      const candidate = this.buildDate(
        Number(isoMatch[1]),
        Number(isoMatch[2]) - 1,
        Number(isoMatch[3]),
      );
      if (!this.isValidDate(candidate)) return null;
      if (this.startOfDay(candidate) < today) return null;
    ...
    const ddDeMes = normalized.match(
      /\b(\d{1,2})\s+de\s+([a-záéíóú]+)(?:\s+de\s+(\d{4}))?\b/u,
    );
    if (ddDeMes) {
  ...
  private buildDate(year: number, month: number, day: number): Date {
