...
async function run() {
  ...
  for (const appt of appointments) {
    const docNumber = (appt as unknown as { patientDocument?: string }).patientDocument;
    ...
    if (!patient) {
      const apptSnapshot = appt as unknown as {
        patientName: string;
        patientEmail?: string;
        patientPhone?: string;
      };
      ...
      patient = await Patient.create({
        ...
        isActive: true,
        observations:
          'Paciente creado automáticamente durante la migración de citas existentes.',
      });
      created++;
    }
