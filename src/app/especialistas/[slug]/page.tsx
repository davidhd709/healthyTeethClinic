...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    ...
    await connectDB();
    const doc = await Specialist.findOne({ slug, isActive: true }).lean();

    if (!doc) {
      return { title: 'Especialista no encontrado | Healthy Teeth Clinic' };
    }

    const specialist: ISpecialist = JSON.parse(JSON.stringify(doc));
...
export default async function EspecialistaDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    _specialist = JSON.parse(JSON.stringify(doc));

    // Fetch related services
    const serviceDocs = await Service.find({
      _id: { $in: _specialist!.services },
      isActive: true,
    }).lean();
    services = JSON.parse(JSON.stringify(serviceDocs));
  } catch (error) {
