...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    ...
    const service: IService = JSON.parse(JSON.stringify(doc));
...
export default async function ServicioDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    _service = JSON.parse(JSON.stringify(doc));

    // Fetch specialists who provide this service
    const specialistDocs = await Specialist.find({
      _id: { $in: _service!.specialists },
      isActive: true,
    }).lean();
    specialists = JSON.parse(JSON.stringify(specialistDocs));
  } catch (error) {
