    ...
    async function fetchSlots() {
      ...
      try {
        const res = await fetch(
          apiUrl(`/api/availability?specialistId=${specialist._id}&date=${dateStr}`)
        );
...
export default function BookingWizard() {
  ...
  useEffect(() => {
    ...
    const serviceId = searchParams.get('servicioId');
    ...
    const hasPrefillParams = Boolean(
      serviceSlug || serviceId || specialistSlug || dateParam || timeParam,
    );
    const hasValidTimeParam = Boolean(timeParam && /^\d{2}:\d{2}$/.test(timeParam));

    if (!hasPrefillParams) return;

    let preselectedService = selectedService;
    let preselectedSpecialist = selectedSpecialist;

    if (!selectedService && (serviceSlug || serviceId)) {
      const match = services.find(
        (s) => s.slug === serviceSlug || s._id === serviceId,
      );
      if (match) {
        setSelectedService(match);
        preselectedService = match;
      }
    }
    ...
    if (specialistSlug && !selectedSpecialist) {
      ...
      if (match) {
        ...
        preselectedSpecialist = match;
      }
    }

    const parsedDate = parseDateParam(dateParam);
    if (parsedDate && !selectedDate && preselectedSpecialist) {
      setSelectedDate(parsedDate);
    }

    if (hasValidTimeParam && !selectedTime && parsedDate && preselectedSpecialist) {
      setSelectedTime(timeParam);
    }

    setCompletedSteps((prev) => {
      const next = new Set(prev);
      if (preselectedService) next.add(1);
      if (preselectedService && preselectedSpecialist) next.add(2);
      if (
        preselectedService &&
  ...
  useEffect(() => {
    ...
    const specialistStillAvailable = filteredSpecialists.some(
      (sp) => sp._id === selectedSpecialist._id,
    );
    if (!specialistStillAvailable) {
      ...
      toast.error('El especialista seleccionado no ofrece ese servicio.');
    }
  }, [selectedService, selectedSpecialist, filteredSpecialists]);
  ...
  const isStepValid = useCallback(
    (step: number): boolean => {
      switch (step) {
        ...
        case 4: {
          return (
            watchedPatientName.trim().length >= 3 &&
            watchedPatientEmail.includes('@') &&
            watchedPatientPhone.trim().length >= 7 &&
            watchedReasonForVisit.trim().length >= 10 &&
            watchedDataConsent === true
          );
        }
        case 5:
          return true;
